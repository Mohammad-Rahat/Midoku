...
nonisolated enum PersonalStatus: String, SettingChoice {
    case planned, reading, completed, onHold, dropped
    var title: String { self == .onHold ? "On hold" : rawValue.capitalized }
}

nonisolated struct LibraryListing: Codable, Identifiable, Sendable {
    var id = UUID()
    let identity: SourceListingIdentity
    var details: MangaDetails
    var refreshedAt: Date?
    var lastError: String?
}

nonisolated struct LibraryChapter: Codable, Identifiable, Sendable {
    var id = UUID()
...
nonisolated struct EntrySourceLink: Codable, Identifiable, Sendable {
    var id = UUID()
    var listingID: UUID
...
nonisolated struct ChapterEdits: Codable, Sendable {
    var title: String?
    var number: String?
    var volume: String?
    var coverID: UUID?
...
nonisolated struct ChapterVariant: Codable, Identifiable, Sendable {
    var id = UUID()
    var chapterID: UUID
    var edits = ChapterEdits()
}

nonisolated struct ChapterSlot: Codable, Identifiable, Sendable {
    var id = UUID()
    var variants: [ChapterVariant]
    var preferredID: UUID
    var completionOverride: Bool?
    var preferred: ChapterVariant? { variants.first { $0.id == preferredID } }
    init(variant: ChapterVariant) { variants = [variant]; preferredID = variant.id }
}

nonisolated struct PersonalEntry: Codable, Identifiable, Sendable {
    var id = UUID()
    var createdAt = Date()
    var updatedAt = Date()
    var lastReadAt: Date?
    ...
    var links: [EntrySourceLink] = []
    var slots: [ChapterSlot] = []
    var exclusions: Set<UUID> = []
    var manualOrder = false
    var sequenceRevision = 0
    var readerOverride: ReaderPreferences?
    var descendingDisplay = false
}

nonisolated struct CopiedChapter: Codable, Identifiable, Sendable {
...
nonisolated struct LibraryCover: Codable, Identifiable, Sendable {
    var id = UUID()
    var data: Data
    var digest: String
    init(data: Data) {
...
nonisolated struct LibraryUpdate: Codable, Identifiable, Sendable {
    var id = UUID()
    var entryID: UUID
    var chapterID: UUID
...
nonisolated enum PasteAction: String, CaseIterable, Identifiable, Sendable {
...
nonisolated struct PasteChoice: Identifiable, Sendable {
    var id: UUID { item.chapterID }
    var item: CopiedChapter
...
nonisolated struct LibraryState: Codable, Sendable {
    var entries: [PersonalEntry] = []
    var listings: [LibraryListing] = []
    var chapters: [LibraryChapter] = []
    var covers: [LibraryCover] = []
    var clipboard: [CopiedChapter] = []
    var completed: Set<SourceChapterIdentity> = []
    var updates: [LibraryUpdate] = []

    func entry(_ id: UUID) -> PersonalEntry? { entries.first { $0.id == id } }
    func listing(_ id: UUID?) -> LibraryListing? { listings.first { $0.id == id } }
    func chapter(_ id: UUID) -> LibraryChapter? { chapters.first { $0.id == id } }
    func title(_ entry: PersonalEntry) -> String { entry.titleOverride ?? listing(entry.primaryListingID)?.details.title ?? "Untitled entry" }
    ...
    @discardableResult
    mutating func remember(details: MangaDetails, connectionID: UUID, records: [ChapterRecord], complete: Bool, language: String? = nil) throws -> UUID {
        let identity = SourceListingIdentity(connectionID: connectionID, externalID: details.id)
        guard Set(records.map(\.id)).count == records.count else { throw LibraryFailure.invalid }
        let listingID: UUID
        if let index = listings.firstIndex(where: { $0.identity == identity }) {
            ...
            var listing = LibraryListing(identity: identity, details: details)
        ...
        for record in records {
            let key = SourceChapterIdentity(listing: identity, externalID: record.id)
            if let index = chapters.firstIndex(where: { $0.identity == key }) {
                chapters[index].record = record; chapters[index].available = true
            } else { chapters.append(LibraryChapter(identity: key, record: record)) }
        }
        return listingID
    }
    ...
    mutating func createManual(title: String, description: String = "", categories: Set<UUID> = []) throws -> UUID {
        ...
        guard !title.isEmpty, title.count <= 1000 else { throw LibraryFailure.emptyTitle }
        var entry = PersonalEntry(); entry.titleOverride = title; entry.descriptionOverride = description; entry.categoryIDs = categories
        entries.append(entry); return entry.id
    }

    mutating func editEntry(_ id: UUID, _ edit: (inout PersonalEntry) throws -> Void) throws {
        guard let index = entries.firstIndex(where: { $0.id == id }) else { throw LibraryFailure.missing }
    ...
    mutating func refresh(details: MangaDetails, connectionID: UUID, records: [ChapterRecord], language: String?) throws {
        let listingID = try remember(details: details, connectionID: connectionID, records: records, complete: true, language: language)
        let identity = SourceListingIdentity(connectionID: connectionID, externalID: details.id)
    ...
    mutating func copy(_ items: [CopiedChapter]) throws {
        guard !items.isEmpty, items.count <= 5000, Set(items.map(\.id)).count == items.count,
              items.allSatisfy({ chapter($0.chapterID) != nil }) else { throw LibraryFailure.invalid }
        clipboard = items
    }

    func pastePreview(entryID: UUID) throws -> [PasteChoice] {
        guard let entry = entry(entryID) else { throw LibraryFailure.missing }
        let existing = Set(entry.slots.flatMap(\.variants).map(\.chapterID))
        return clipboard.map { item in
            let incoming = ChapterVariant(chapterID: item.chapterID, edits: item.edits)
            ...
            return PasteChoice(item: item, action: existing.contains(item.chapterID) ? .skip : (match == nil ? .separate : .unresolved), targetSlotID: match?.id)
        }
    }

    mutating func paste(entryID: UUID, revision: Int, choices: [PasteChoice], insertBefore: UUID? = nil) throws {
        guard let index = entries.firstIndex(where: { $0.id == entryID }) else { throw LibraryFailure.missing }
        guard entries[index].sequenceRevision == revision else { throw LibraryFailure.stalePreview }
        guard Set(choices.map(\.id)).count == choices.count, choices.allSatisfy({ $0.action != .unresolved }) else { throw LibraryFailure.unresolved }
        var candidate = self
        var entry = candidate.entries[index]
        for choice in choices where choice.action != .skip {
            guard let source = chapter(choice.item.chapterID), let listing = listings.first(where: { $0.identity == source.identity.listing }) else { throw LibraryFailure.missing }
            if entry.slots.flatMap(\.variants).contains(where: { $0.chapterID == source.id }) { continue }
            guard !entry.exclusions.contains(source.id) || choice.restoreExcluded else { throw LibraryFailure.excluded }
            entry.exclusions.remove(source.id)
            if !entry.links.contains(where: { $0.listingID == listing.id }) {
                entry.links.append(EntrySourceLink(listingID: listing.id, followsNewChapters: false, language: source.record.language))
            }
            let variant = ChapterVariant(chapterID: source.id, edits: choice.item.edits)
            switch choice.action {
            case .alternative, .replace:
                guard let slotIndex = entry.slots.firstIndex(where: { $0.id == choice.targetSlotID }) else { throw LibraryFailure.stalePreview }
            ...
            case .separate:
                let slot = ChapterSlot(variant: variant)
    ...
    mutating func restoreChapter(entryID: UUID, chapterID: UUID) throws {
        guard chapter(chapterID) != nil else { throw LibraryFailure.missing }
        try editEntry(entryID) { entry in
            entry.exclusions.remove(chapterID)
            if !entry.slots.flatMap(\.variants).contains(where: { $0.chapterID == chapterID }) {
                entry.slots.append(ChapterSlot(variant: ChapterVariant(chapterID: chapterID)))
            }
        }
    }
    ...
    func sortSequence(_ entry: inout PersonalEntry) {
...
nonisolated enum LibraryFailure: Error, LocalizedError {
