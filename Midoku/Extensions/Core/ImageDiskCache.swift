...
/// Disposable cover bytes only. Never stores cookies, library edits, or downloaded pages.
nonisolated struct ImageDiskCache: Sendable {
    ...
    func store(_ data: Data, key: String) throws {
        guard !data.isEmpty, data.count <= min(maximumBytes, 2 * 1024 * 1024) else { return }
