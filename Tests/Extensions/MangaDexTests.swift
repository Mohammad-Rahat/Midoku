...
struct MangaDexTests {
    @Test func shippedBundleExecutesEveryCapabilityThroughJavaScriptCore() async throws {
        ...
        #expect(manifest.capabilities == [.search, .feeds, .details, .chapters, .pages, .filters])
        ...
        let host = MangaDexFixtureHost(fixtures: ["tags", "search", "search", "details", "chapters", "chapter", "pages"], manifest: manifest)
        ...
        let filters = try await adapter.searchFilters()
        #expect(filters.first(where: { $0.id == "includedTags" })?.options.count == 2)
        #expect(filters.first(where: { $0.id == "sort" })?.scopes == [.search])
        let search = try await adapter.search(query: "Test & Atlas", cursor: nil, filters: ["status": ["completed"], "sort": ["title:asc"]])
        ...
        #expect(details.authors == ["Fixture Author"])
        #expect(details.availableLanguages?.map(\.title) == ["English", "Spanish"])
        #expect(details.tags == ["Adventure"])
        ...
        #expect(chapters.items.first?.groups == ["Fixture Scanlation Group"])
        ...
        #expect(requests[1].url.absoluteString.contains("Test%20%26%20Atlas"))
        let parameters = URLComponents(url: requests[1].url, resolvingAgainstBaseURL: false)?.queryItems ?? []
        #expect(parameters.contains(URLQueryItem(name: "status[]", value: "completed")))
        #expect(parameters.contains(URLQueryItem(name: "order[title]", value: "asc")))
