...
struct ClipboardView: View {
    ...
    var body: some View {
        ...
        .toolbar { ToolbarItem(placement: .topBarTrailing) { Button("Clear") { Task { do { try await settings.commit { $0.library.clipboard = [] } } catch { self.error = error.localizedDescription } } }.disabled(settings.snapshot.library.clipboard.isEmpty) } }
