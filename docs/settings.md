...
The subsequent [Xcode 27 GitHub Actions build](https://github.com/Mohammad-Rahat/Midoku/actions/runs/35388694209) passed all 34 Swift tests in nine suites and successfully archived the iOS app in Release configuration. This native check found and fixed a missing UniformTypeIdentifiers import and settings identifier actor-isolation warnings. The IPA is unsigned; see [IPA builds](ipa-build.md) for delivery and signing details. Remaining compiler warnings include the pre-existing JavaScript runtime initializer isolation and deprecated UIScreen.main brightness access.

Required device checks remain: light/dark and accessibility sizes; all settings routes; source pin filters/back navigation; source/continuous/offline reader mode and zoom; restore preview/recovery and relaunch; lock cancellation/passcode fallback/delay while a sheet is open; Wi-Fi changes, pause/cancel/relaunch, disk failure, corrupt page recovery, cache clear followed by airplane-mode reading. No simulator or physical-device acceptance pass has been performed for these settings changes. Preserve the existing signing and deployment settings.
