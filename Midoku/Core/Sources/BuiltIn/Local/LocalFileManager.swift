...
extension LocalFileManager {
    ...
    func uploadFile(
        ...
        if !hasMangaObject {
            let cover = coverURL?.toMidokuImageUrl()?.absoluteString ?? {
                ...
                for ext in Self.allowedImageExtensions {
                    ...
                    if coverPath.exists {
                        return coverPath.toMidokuImageUrl()?.absoluteString
                    }
                }
                return nil
            }()
...
extension LocalFileManager {
    func setCover(for mangaKey: String, image: PlatformImage) async -> String? {
        ...
        if let cover = mangaData?.cover, let url = URL(string: cover) {
            let fileURL = url.toMidokuFileUrl() ?? url
        ...
        return await CoreDataManager.shared.setCover(
            ...
            coverUrl: newCoverURL.toMidokuImageUrl()?.absoluteString
        )
    }
}
