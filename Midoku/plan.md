...
The user supplied the UI design kit and approved Midoku asset pack on 2026-09-18. Repository copies live in `docs/design/manga-reader-design-kit/` and `docs/design/Midoku/` (paths relative to the repository root). Read `docs/design/README.md` for precedence and integration. Use the UI kit for layouts and the newer brand guide for final identity, adaptive colors, and illustrations. Native resources are in `Midoku/Resources/MidokuAssets.xcassets`; shared SwiftUI components are in `Midoku/DesignSystem/`.
