...
- Local `npm ci && npm test` passes all 21 checks. Added native tests for reviewed hosts/lookalikes, preload boundaries/failure/cancellation, shared-task cancellation, request priority, independent disk limits, and grid-style migration/backup round trips. Native tests, archive, simulator screenshot review, and expanded live image decoding probe run in CI; physical-device verification remains required.
