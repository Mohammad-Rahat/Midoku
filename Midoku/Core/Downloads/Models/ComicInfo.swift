...
struct ComicInfo: Hashable {
    ...
    struct MidokuNotesData: Codable {
...
extension ComicInfo {
    func extraData() -> MidokuNotesData? {
        guard
            let notes,
            notes.hasPrefix(MidokuNotesData.prefix)
        else {
            return nil
        }
        return try? JSONDecoder().decode(
            MidokuNotesData.self,
            from: Data(notes.dropFirst(MidokuNotesData.prefix.count).utf8)
        )
    }
}
...
extension ComicInfo {
    static func load(manga: AidokuRunner.Manga, chapter: AidokuRunner.Chapter) -> ComicInfo {
        ...
        return .init(
            ...
            notes: (try? JSONEncoder().encode(MidokuNotesData(
                sourceKey: manga.sourceKey,
                mangaKey: manga.key,
                chapterKey: chapter.key
            ))).flatMap {
                String(data: $0, encoding: .utf8).flatMap { MidokuNotesData.prefix + $0 }
            },
