...
- Added real presentation/retry regression cases and memory-profile checks in the native WebKit probe. [Probe 35459724067](https://github.com/Mohammad-Rahat/Midoku/actions/runs/35459724067) passed memory-store isolation, native cookie headers, independent reset and actual required-frame execution. Live memory and persistent modes both still returned 403, including native retry and bundled extension search. The LiveContainer profile change remains a compatibility hypothesis requiring device confirmation. See [verification details](cloudflare-verification.md).
- [IPA workflow 35459724125](https://github.com/Mohammad-Rahat/Midoku/actions/runs/35459724125), source commit `6149f3889233e127994e2fca9497b5e999e1cdca`, passed 87 Swift tests in 22 suites, including both explicit-retry outcomes, all 27 Node tests, the Release archive and Debug simulator build/capture step. No physical-device/LiveContainer success has been established.
- Delivered `Midoku-cloudflare-r5-unsigned.ipa`, 4,452,655 bytes; SHA-256 `98cb42acea0954d9fcf9c0e6392bf95a6849f5e5a07330f86c5f66dc77b0b3d5`. Verified archive/workflow checksums and source commit, ZIP integrity, arm64 executable, revision-5 marker and absence of signing/provisioning data. Bundle ID `com.raahat.Midoku` and minimum iOS 27 remain unchanged. On the device, Connection details should show `Session: temporary`; fresh clearance enables Try request if the browser continues cycling.
...
- Revision-4 connection details include fixed blocked-frame categories without exposing URLs, cookies or tokens.
- [Revision-4 live probe 35453048332](https://github.com/Mohammad-Rahat/Midoku/actions/runs/35453048332) passed actual nested-frame execution and recorded zero blocked live navigations. Both browser configurations still received HTTP 403 on the Mac runner. The native retry returned 403 and the bundled NovelCrow search failed with HTTP 403. A green diagnostic workflow is not a successful live source check; the runner's remaining 403 cause is unestablished. Physical-device/LiveContainer clearance remains unverified.
- [IPA workflow 35453048372](https://github.com/Mohammad-Rahat/Midoku/actions/runs/35453048372), source commit `ce6dd6a7ee26e48cacdddca8dbfd5fec52a7141c`, passed 85 Swift tests in 21 suites, all 27 Node tests, the Release archive and Debug simulator build/capture step with Xcode 27. The new actual-WebKit regression passed in 1.574 seconds. These results do not establish live NovelCrow success.
- Delivered `Midoku-cloudflare-r4-unsigned.ipa`, 4,450,075 bytes; SHA-256 `0121fd6d2c462ad033f787ac6a95839ff2cfbf86b885945ba2257232ff66ffd8`. Verified archive/workflow checksums, source commit, ZIP integrity, arm64 executable, revision-4 marker and absence of signing/provisioning data. Bundle ID `com.raahat.Midoku` and minimum iOS 27 remain unchanged.
...
- Added eight regression tests for frame policy, request ownership, DOM/cookie state and completion races. CI also exposed a CRLF grapheme edge case in header validation; the native and verification paths now reject CR/LF bytes directly. Corrected the test macro usage and DOM fixture before the successful run.
- [Workflow 35444429961](https://github.com/Mohammad-Rahat/Midoku/actions/runs/35444429961), source commit `98e6d9c632966970c168396e0e467e282929165d`, passed all 83 Swift tests in 20 suites, 27 Node checks, the Release archive, and the Debug simulator build/capture step with Xcode 27.0. These checks do not exercise a production challenge on the user's device. See [implementation and device checklist](cloudflare-verification.md).
- Delivered `Midoku-cloudflare-r3-unsigned.ipa`, 4,448,529 bytes; SHA-256 `719a00ae8083cf5c9f8f1d10ea954b787adc6f5f6fb597a2cca76df864f1bf7c`. Verified ZIP integrity, workflow checksum/source commit, arm64 executable, revision-3 verification code, and absence of a signature load command, signature directory or provisioning profile. Bundle ID `com.raahat.Midoku` and minimum iOS 27 are unchanged. LiveContainer clearance and the complete NovelCrow image path still need device acceptance.
...
- Final verification: [workflow 35436350042](https://github.com/Mohammad-Rahat/Midoku/actions/runs/35436350042) passed 75 Swift tests in 19 suites, extension tooling, the Release device archive and all 27 simulator captures. Reviewed native tabs, Home updates visibility/settings, Library category/search/actions with slate/light and ochre/dark accents, the add-entry form, compact entries, bottom content clearance and reader tab hiding. Screenshot review caught a reading-button contrast issue; the final build uses a stable accent fill and light foreground, removes duplicate status/category labels and trims section spacing. These are screenshot checks, not automated touch-flow or physical-device tests.
- Delivered unsigned arm64 IPA from `fd7cb8a023f1a79105c280a35b04bb103d62737b`, 4,428,149 bytes; SHA-256 `8e4e439d7f6802e2ec9676e6cb872a982f46ade83d5b010f7f53c77ee0e89fb3`. ZIP integrity and workflow checksum match; executable has no code-signature command, signature directory or embedded provisioning profile. Bundle identifier remains `com.raahat.Midoku`, minimum iOS remains 27, and project signing settings are unchanged.
...
### Comix image hosts, page preloading, and grid presentation

- The live One-Punch Man probe returned releases on both `jloo.wowpic1.store` and `jdpw.wowpic1.store`, alongside the already-reviewed wowpic2 shards. Comix 0.1.1 adds the bounded `*.wowpic1.store` permission. Whole-page-list validation previously rejected those releases, preventing both the reader and first-page thumbnails. HTTPS, header, redirect and host-boundary checks remain enforced.
- The online reader preloads exactly the next five physical pages after its current page loads. Page/chapter changes, leaving the reader and backgrounding cancel the old window. Preloads never update progress/completion or open verification. Visible requests precede queued preloads and thumbnail work. Coalesced requests cancel their transport only when no caller remains; visible callers can retry an interactive challenge rejected by background work.
- Reader pages share a 64 MiB memory budget and a separate disposable 192 MiB / 24-hour disk cache (32 MiB per item), retaining processed image bytes without lossy recompression. Clear cache clears both reader and cover caches without touching offline downloads or custom covers. Prefetch errors are retried when a page is actually opened.
- Chapter grids now support Default (up to three single-line, tail-truncated details), Compact (chapter name over the cover) and Thumbnail (cover only), independently of portrait/landscape column counts. Existing settings/backups default to the existing presentation. Chapter action dots are removed from both list and grid rows; long press keeps all actions, with accessible actions retained.
- Verification completed: 21 Node checks and 71 Swift tests pass. Release archive and Debug simulator capture run `35433507026` succeeded at app commit `d9874aac7a88d7153e7ab5f22e3f2d377996e3cd`. Reviewed Default truncation, Compact title overlays, Thumbnail-only cards, and the settings controls; chapter dots are absent. Regression coverage includes reviewed hosts/lookalikes, preload boundaries/failure/cancellation, shared-task cancellation, request priority, independent disk limits, and grid-style migration/backup round trips.
- Live probe `35433507045` decoded first-page image bytes for all eight sampled One-Punch Man physical releases through the production request coordinator and Comix decoder, including both observed wowpic1 shards. This does not establish availability of every chapter or physical-device challenge completion; LiveContainer and touch interaction testing remain with the user.
- Packaged `Midoku-comix-preload-unsigned.ipa` (4,406,810 bytes), SHA-256 `1e3410e0cd03e560034d6f7c00ec8ecb7c58fb0eb1278db38cf3dcb6e5b3d592`. ZIP, checksum, arm64 Mach-O, absent signing load command/profile, bundled CDN permission, bundle ID and unchanged minimum OS 27 verified.
...
- Added settings migration, restart persistence and invalid-backup count checks, plus simulator captures for portrait chapter lists, chapter grids, source grids and empty clipboard state. Native verification passed 63 Swift tests and 21 extension tests; the Release archive and Debug simulator build succeeded. Screenshot review caught native List disclosure accessories shrinking chapter grid covers; grid cards now navigate through independent buttons and destinations while list rows retain native disclosure.
